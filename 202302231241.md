...
**Note**: For storing attributes with very large value, we can utilise the type
`clob({size})` (character large object) and `blob({size})` (binary large
object).
...
[referential integrity constraint](202302101331.md)), `not null`, `unique`, and
`check` clause, can be enforced on an attribute (column). Other than that, we
can set a default value for an attribute with `default {value}` immediately
after the attribute definition. The following SQL codes shows such use case:
...
preserve tuples with no entry in other relation. The following codes shows such
use case:
...
that can be applied on attributes (`avg` and `sum` can only be applied on a
collection of numbers). We can improve the query by grouping in specific
attributes instead of applying on the entire relation with the command `group
by`. Together with another command `having` (applying constraints on those
grouped), we can have the following query with highly readable:
...
- the `select` clause contains simple attribute names (no expressions,
...
disable` or drop altogether with `dropo trigger {trigger-name}`. Despite its
